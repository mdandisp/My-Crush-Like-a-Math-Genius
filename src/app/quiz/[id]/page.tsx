...
function QuizContent({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [attemptDetails, setAttemptDetails] = useState<AttemptSession | null>(null);
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        ...
        const topicsRes = await fetchApi('/api/v1/topics');
        if (topicsRes && topicsRes.data) {
          const mapped = mapTopicsToCharacters(topicsRes.data, targetType);
        ...
        const res = await fetchApi(`/api/v1/topics/${resolvedParams.id}/attempts/info`);
  ...
  const startQuiz = async () => {
    ...
    const levelSettings = topicInfo?.level_settings?.find((l: any) => l.level === level);
    if (levelSettings && requestedCount > levelSettings.remaining_questions) {
      toast.error(`Jumlah soal melebihi sisa soal (${levelSettings.remaining_questions})`);
      return;
    }
          ...
          {quizState === 'info' && (
            <div className="animate-fade-in quiz-info-state-container" style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
              ...
              <div style={{
                ...
                {isLoadingInfo ? (
                  <p style={{ color: '#a0a5b5', textAlign: 'center' }}>Memuat info...</p>
                ) : (
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', marginTop: '2rem' }}>
                ...
                <button
                  ...
                  onClick={startQuiz}
                  disabled={isLoadingInfo || isSubmitting || topicInfo?.remaining_attempts === 0}
                  style={{ opacity: (isLoadingInfo || isSubmitting || topicInfo?.remaining_attempts === 0) ? 0.5 : 1 }}
                >
            ...
            <div className="animate-fade-in" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
                <button
                  ...
                  style={{ opacity: (!selectedAnswer || isSubmitting) ? 0.5 : 1, cursor: (!selectedAnswer || isSubmitting) ? 'not-allowed' : 'pointer' }}
                >
                  {isSubmitting ? 'Mengirim...' : (currentQ === totalQ - 1 ? 'Selesai' : 'Lanjut')}
                </button>
              </div>
            </div>
          )}
...
export default function QuizPage({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', backgroundColor: '#0f1015', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>Memuat...</div>}>
